...
extern "C" int add_6(int a, int b, int c, int d, int e, int f);
extern "C" uint32_t asm_led(uint32_t a);
extern "C" uint32_t asm_error(uint32_t a, uint32_t b);
...
void part2() {
  ...
  // Static local variables can only be initialized once, and the storage space is in different areas
}
...
void part3() {
	...
	static uint32_t val2 = 0x99999999;  // set this to something that will cause an overflow
  static uint8_t ledVar = 0;

	// write your solution here:
  ledVar = asm_error(val1,val2);
